...
export default function DeliveryNotesPage() {
  ...
  const fetchDeliveryNotes = async () => {
    try {
      ...
      setError(err.response?.data?.detail || "Erreur lors du chargement des bons de livraison");
    } finally {
      setLoading(false);
    }
  };
    ...
    <DashboardLayout title="Bons de livraison">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground">{note.number}</h3>
                      <p className="text-sm text-accents-5">{note.client_name || note.client_id}</p>
                    </div>
                  </div>
                </div>
      ...
      {!loading && deliveryNotes.length === 0 && (
        <Card className="p-12 text-center">
          <p className="text-accents-5">Aucun bon de livraison trouvé</p>
        </Card>
      )}
    </DashboardLayout>
  );
